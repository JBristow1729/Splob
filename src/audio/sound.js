export class Sound {
  ...
  static play(kind = "tap", value = 0) {
    ...
    if (kind === "swishoo" || kind === "fanfare") {
      ...
      osc.frequency.setValueAtTime(freq * 1.35, ctx.currentTime);
      osc.frequency.exponentialRampToValueAtTime(freq * 0.62, ctx.currentTime + duration);
    }
    osc.type = kind === "fart" ? "square" : kind === "splat" || kind === "thud" || kind === "score" ? "sawtooth" : "triangle";
    gain.gain.setValueAtTime(Sound.volume * (kind === "fart" ? 0.2 : kind === "thud" || kind === "score" ? 0.24 : 0.16), ctx.currentTime);
